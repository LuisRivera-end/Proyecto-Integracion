      ...
      <div class="w-[26rem] flex-shrink-0 flex flex-col gap-5">
        ...
        <div class="bg-white/95 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.15)] border border-white/20 p-8 flex-1">
            <div class="flex justify-center mb-6">
                <h2 class="text-slate-400 text-sm font-bold uppercase tracking-widest text-center">
                    Tickets en espera por sector
                </h2>
            </div>
            
            <div id="sector-badges" class="grid grid-cols-2 gap-x-4 gap-y-6">
                <div v-for="(cantidad, sector) in sectorCounts" :key="sector" class="flex flex-col gap-2">
                  <span class="text-sm font-bold uppercase tracking-wider text-slate-500">
                    {{ getSectorLabel(String(sector)) }}
                  </span>
                  <div class="flex items-center justify-center gap-4 px-4 py-4 rounded-xl border-2 transition-all duration-200"
                       :class="getSectorTheme(String(sector)).cardClasses">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                         :class="getSectorTheme(String(sector)).iconBg">
                      <svg class="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
                        <path :d="getSectorTheme(String(sector)).iconPath" />
                      </svg>
                    </div>
                    <span class="text-5xl font-black" :class="getSectorTheme(String(sector)).numberColor">{{ cantidad }}</span>
                  </div>
                </div>
            </div>

            <!-- Mensaje cuando no hay tickets -->
            <div v-if="Object.keys(sectorCounts).length === 0" class="flex flex-col items-center justify-center py-8 text-slate-400">
              <svg class="w-10 h-10 mb-2 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span class="text-sm font-medium">Sin tickets en espera</span>
            </div>
        </div>
...
/**
 * Configuración visual por sector: colores, ícono SVG y clases Tailwind.
 */
...
const sectorThemes: Record<string, SectorTheme> = {
  cajas: {
    cardClasses: 'border-[#1ed760] bg-[#1ed760]/5',
    iconBg: 'bg-[#1ed760]',
    iconPath: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z',
    numberColor: 'text-[#047857]',
  },
  'servicios escolares': {
    cardClasses: 'border-[#3b82f6] bg-[#3b82f6]/5',
    iconBg: 'bg-[#3b82f6]',
    iconPath: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253',
    numberColor: 'text-[#1d4ed8]',
  },
  becas: {
    cardClasses: 'border-[#f59e0b] bg-[#f59e0b]/5',
    iconBg: 'bg-[#f59e0b]',
    iconPath: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
    numberColor: 'text-[#92400e]',
  },
  tesoreria: {
    cardClasses: 'border-[#ef4444] bg-[#ef4444]/5',
    iconBg: 'bg-[#ef4444]',
...
const defaultTheme: SectorTheme = {
  cardClasses: 'border-slate-300 bg-slate-50',
  iconBg: 'bg-slate-500',
...
function getSectorTheme(sector: string): SectorTheme {
  ...
  for (const [k, v] of Object.entries(sectorThemes)) {
...
function getSectorLabel(sector: string): string {
  ...
  if (lower.includes('servicios escolares')) return 'Serv. Escolares'
  if (lower.includes('tesoreria') || lower.includes('tesorería')) return 'Tesorería'
  return sector
}
/**
