...
let empleadosGlobal = [];
let sectorActual = "cajas";


  // -----------------------------
  // MOSTRAR EMPLEADOS SEGÚN SECTOR
  // -----------------------------
  function populateEmployeeSelect(empleados) {
    ...
    // Filtrar empleados por sector seleccionado
    const empleadosFiltrados = empleados.filter(emp => {
      if (sectorActual === "cajas") return emp.rol_id === 2;
      if (sectorActual === "becas") return emp.rol_id === 3;
      if (sectorActual === "servicios_escolares") return emp.rol_id === 4;
      return false;
    });
...
function selectSector(sector) {
  ...
  if (empleadosGlobal.length > 0) {
    populateEmployeeSelect(empleadosGlobal);
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (agendaForm) {
    ...
    for (let d of descansosElems) {
      const inicio = d.querySelector("[id^='inicio-']").value;
      const fin = d.querySelector("[id^='fin-']").value;

      if (!inicio || !fin) {
        alert("Todos los descansos deben tener fecha de inicio y fin");
        return;
      }

      descansos.push({ inicio, fin });
    }
